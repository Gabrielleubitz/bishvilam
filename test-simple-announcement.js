const admin = require('firebase-admin');

// Initialize Firebase Admin
if (!admin.apps.length) {
  try {
    const serviceAccountB64 = process.env.FIREBASE_SERVICE_ACCOUNT_B64 || '';
    const serviceAccount = JSON.parse(Buffer.from(serviceAccountB64, 'base64').toString('utf8'));
    
    admin.initializeApp({
      credential: admin.credential.cert(serviceAccount)
    });
    console.log('✅ Firebase Admin initialized successfully');
  } catch (error) {
    console.error('❌ Firebase initialization error:', error.message);
    process.exit(1);
  }
}

const db = admin.firestore();

async function checkDatabaseContent() {
  console.log('🔍 Checking current database content...\n');

  try {
    // Check announcements
    console.log('📢 Checking announcements:');
    const announcements = await db.collection('announcements').get();
    console.log(`Found ${announcements.size} announcements:`);
    
    announcements.docs.forEach((doc, index) => {
      const data = doc.data();
      console.log(`${index + 1}. "${data.title}"`);
      console.log(`   - Target Groups: ${data.targetGroups?.join(', ') || 'None'}`);
      console.log(`   - Type: ${data.type}`);
      console.log(`   - Active: ${data.active}`);
      console.log(`   - Email Sent: ${data.emailSent}`);
      console.log('');
    });

    // Check profiles
    console.log('👤 Checking user profiles:');
    const profiles = await db.collection('profiles').get();
    console.log(`Found ${profiles.size} user profiles:`);
    
    profiles.docs.forEach((doc, index) => {
      const data = doc.data();
      console.log(`${index + 1}. ${data.firstName || 'Unknown'} ${data.lastName || ''} (${data.email})`);
      console.log(`   - Groups: ${data.groups?.join(', ') || 'None'}`);
      console.log(`   - Role: ${data.role || 'student'}`);
      console.log('');
    });

    // Test email API directly
    console.log('📧 Testing email API with test announcement...');
    
    if (announcements.size > 0) {
      const firstAnnouncement = announcements.docs[0];
      const announcementData = firstAnnouncement.data();
      
      const emailTestData = {
        announcementId: firstAnnouncement.id,
        title: announcementData.title,
        content: announcementData.content,
        targetGroups: announcementData.targetGroups,
        type: announcementData.type
      };

      try {
        const response = await fetch('http://localhost:3001/api/send-announcement-email', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify(emailTestData),
        });

        if (response.ok) {
          const result = await response.json();
          console.log('✅ Email API test successful:', result.message);
          console.log('📊 Email stats:', result.details);
        } else {
          const error = await response.json();
          console.log('⚠️ Email API test failed:', error.error);
        }
      } catch (error) {
        console.log('❌ Email API test error:', error.message);
      }
    } else {
      console.log('❌ No announcements found to test with');
    }

    console.log('\n✅ Database check completed!');
    console.log('\n📋 Next Steps:');
    console.log('1. Open http://localhost:3001/admin in your browser');
    console.log('2. Login and go to "ניהול הודעות" tab');
    console.log('3. You should see the announcements listed above');
    console.log('4. Try creating a new announcement and sending emails');
    console.log('5. Open http://localhost:3001/account');
    console.log('6. Login with different test users to see filtered announcements');

  } catch (error) {
    console.error('❌ Database check error:', error.message);
  }
}

checkDatabaseContent();